...
let list = csvJSON(fs.readFileSync('list.csv', 'utf-8'));

var { sonos_room, group_with, sonos_http_api, reset_repeat, reset_shuffle, reset_crossfade } = JSON.parse(
...
const get_sonos_url = (sonos_instruction, service_type) => {
  ...
  if (service_type == 'path') {
    return sonos_http_api + sonos_instruction;
  }
  return sonos_http_api + '/' + sonos_room + '/' + sonos_instruction;
};

// we replace the queue if the command is a repeat of the last command. otherwise we add to the queue
// we can also add a command to clear the queue if we want to start fresh
...
module.exports = async function process_sonos_command(received_text = '') {
  ...
  if (command_text == last_command) {
    console.log('Command is a repeat of the last command. Replacing queue.');
    keep_queue = false;
  }
  last_command = command_text;

  let group = true; // i always want to group these two but i might change my mind later so let's keep it as a variable
  let shuffle = received_text_split?.[1]?.includes('shuffle') || false;

  if (command_text.startsWith('spotify:')) {
    service_type = 'spotify';
    sonos_instruction = `spotify/${keep_queue ? 'queue' : 'now'}/${command_text}`
  } else if (command_text.startsWith('path:')) {
    service_type = 'path';
    sonos_instruction = command_text.slice(5);
  } else if (command_text.startsWith('apple:')) {
    service_type = 'applemusic';
    sonos_instruction = `applemusic/${keep_queue ? 'queue' : 'now'}/${command_text.slice(6)}`;
  } else if (command_text.startsWith('applemusic:')) {
    service_type = 'applemusic';
    sonos_instruction = `applemusic/${keep_queue ? 'queue' : 'now'}/${command_text.slice(11)}`;
    ...
    sonos_instruction = `amazonmusic/${keep_queue ? 'queue' : 'now'}/${command_text.slice(12)}`;
  ...
  let metadata = list.find(e => e.Webhook == command_text);
  if (metadata?.Album) {
    console.log('playing album: ' + metadata.Album + ' by ' + metadata.Artist);
  }
  ...
  if (service_type !== 'command') {
    ...
    if (group && group_with.length > 0) {
      console.log('Grouping speakers');
      for (group_with_speaker of group_with) {
        res = await fetch(get_sonos_url('/join/' + group_with_speaker));
        if (!res.ok)
          console.error(
            `Unexpected response while adding speaker ${group_with_speaker}: ${res.status}`
          );
      }
    } 
